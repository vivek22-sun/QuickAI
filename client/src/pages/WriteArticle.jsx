...
const WriteArticle = () => {
    ...
    const downloadPDF = async () => {
        ...
        try {
            ...
            const response = await axios.post('/api/ai/generate-article-pdf', 
                { content, title }, 
                {
                    headers: { Authorization: `Bearer ${await getToken()}` },
                    responseType: 'blob'
                ...
                <div className='flex items-center justify-between'>
                    ...
                    {content && (
                        <button
                            onClick={downloadPDF}
                            disabled={pdfLoading}
                            className='flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 text-sm rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed'
                        >
                            {pdfLoading ? (
